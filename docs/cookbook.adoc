...
Assuming a separate alias (probably in your `~/.clojure/deps.edn`) to invoke the build, you can then execute the build by combining that with the build config in `:jar`:
